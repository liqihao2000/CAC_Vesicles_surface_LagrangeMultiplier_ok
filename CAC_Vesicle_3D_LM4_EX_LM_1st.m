function [phi,eta,xi] = CAC_Vesicle_3D_LM4_EX_LM_1st(pde,domain,Nx,Ny,Nz,time,option)
...
%         tmp  = ephi_t./M - lap_diff(emu);
    ...
    g2 = -eta0.*(delta_Q(phi_star)-fun_inner(1,delta_Q(phi_star))./(Lx*Ly*Lz));
    ...
    % Step 3
    xi = fun_inner(delta_B(phi_star),phi0-phi1_new-phi2_new) ./ fun_inner(delta_B(phi_star),phi3_new);

    xi = fsolve(@(xi)non_fun_xi(xi,phi1_new,phi2_new,phi3_new,phi00),xi,opts);

%     eta_initial = 1; 
%     eta = fsolve(@(eta)non_fun_eta(eta,xi,phi1_new,phi2_new,phi3_new,phi0),eta_initial,opts);
    
    % Step 4
    phi = phi1_new + phi2_new + xi*phi3_new; 
    eta = fun_eta(eta0,alpha,xi,phi,phi0);
...
function r = fun_eta(eta0,alpha,xi,phi,phi0)
    left = fun_inner(1,fun_Q(phi)-fun_Q(phi0));
    right = eta0.*fun_inner(delta_Q(phi0),phi-phi0)...
            -xi.*fun_inner(delta_B(phi0),phi-phi0);
    r = eta0 + alpha * (-left + right);
end
