function [phi,eta,xi] = CAC_Vesicle_3D_LM4_EX_LM_BDF(pde,domain,Nx,Ny,Nz,time,option)
...
[phi1,eta1,~] = CAC_Vesicle_3D_LM4_EX_LM_1st(pde,domain,Nx,Ny,Nz,time1,option);
...
%         tmp  = ephi_t./M - lap_diff(emu);
    ...
    % step 1
    g2 = -eta_star.*(delta_Q(phi_star)-fun_inner(1,delta_Q(phi_star))./(Lx*Ly*Lz));
    ...
    % Step 3
    xi = fun_inner(delta_B(phi_star),4*phi1-phi0-3*(phi1_new+phi2_new)) ./ fun_inner(delta_B(phi_star),3*phi3_new);
    xi = fsolve(@(xi)non_fun_xi(xi,phi1_new,phi2_new,phi3_new,phi00),xi,opts);
    
    % Step 4
    phi = phi1_new + phi2_new + xi.*phi3_new; 
    eta = fun_eta(eta1,eta0,alpha,xi,phi,phi1,phi0);
    ...
    if 1 == option.energyflag
        calculate_energy(out1,out2,t,phi1,phi0,eta1,eta0,alpha)
    end

    if  0 == mod(nt,nsave)
        if 1 == option.printflag
            timeElapsed = toc(tstart);
            fprintf('xi=%.4e,eta=%.4e,epsilon=%.3f,t=%.5f/%.4f, dt=%.2e, Nx=%d, Ny=%d, Nz=%d, timeElapsed=%f\n',xi,eta,epsilon,t,T,dt,Nx,Ny,Nz,timeElapsed);
        end
...
function [] = calculate_energy(out1,out2,t,phi1,phi0,eta1,eta0,alpha)
...
function r = non_fun_xi(xi,phi_new1,phi_new2,phi_new3,phi0)
    left = B(phi_new1+phi_new2+xi.*phi_new3);
    right = B(phi0);
    r = left - right;
end

function r = fun_eta(eta1,eta0,alpha,xi,phi,phi1,phi0)
    phi_star = 2*phi1-phi0; 
    eta_star = 2*eta1-eta0;
    left = fun_inner(1,3*fun_Q(phi)-4*fun_Q(phi1) + fun_Q(phi0));
    right = eta_star.*fun_inner(delta_Q(phi_star),3*phi-4*phi1+phi0)...
            -xi.*fun_inner(delta_B(phi_star),3*phi-4*phi1+phi0);
    r = (4*eta1 - eta0 + alpha * (-left + right)) ./ 3;
end
