...
for kkk = 1:2
    ...
    for kk = 1:size(dirname,1)
        ...
        name=[char(dirname(kk)),'phi_e',num2str(para.epsilon),...
            'M',num2str(para.M),'S1=',num2str(para.S1),'Nx=',num2str(N),'Ny=',num2str(N)];
        A = readtable([name,'.txt']);
        dt_array  = A.dt_array;
        error_phi = A.error;

        if 1 == kk
            ax=loglog(dt_array,dt_array*90,'k-.','linewidth',3);
            hold on;
            grid on;
            ax=loglog(dt_array,dt_array.^2*70000,'k:','linewidth',3);
