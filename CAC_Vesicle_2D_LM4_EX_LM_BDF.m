function [phi,eta,xi] = CAC_Vesicle_2D_LM4_EX_LM_BDF(pde,domain,Nx,Ny,time,option)
...
[phi1,eta1,~] = CAC_Vesicle_2D_LM4_EX_LM_1st(pde,domain,Nx,Ny,time1,option);
...
%         tmp  = ephi_t./M - lap_diff(emu);
    ...
    % step 1
    g2 = - eta_star.*(delta_Q(phi_star)-fun_inner(1,delta_Q(phi_star))./(Lx*Ly));
    ...
    if  0 == mod(nt,nsave)
        if 1 == option.printflag
            ...
            fprintf('xi=%.4e,eta=%.4e,epsilon=%.3f,t=%.5f/%.4f, dt=%.2e, Nx=%d, Ny=%d, timeElapsed=%f\n',xi,eta,epsilon,t,T,dt,Nx,Ny,timeElapsed);
        end
