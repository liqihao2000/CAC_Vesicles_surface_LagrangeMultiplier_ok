function [phi,eta,xi] = CAC_Vesicle_2D_LM4_EX_LM_1st(pde,domain,Nx,Ny,time,option)
...
%         tmp  = ephi_t./M - lap_diff(emu);
    ...
    g2 = -eta0.*(delta_Q(phi_star)-fun_inner(1,delta_Q(phi_star))./(Lx*Ly));
    ...
    xi = fsolve(@(xi)non_fun_xi(xi,phi1_new,phi2_new,phi3_new,phi00),xi,...
                   opts);

%     eta_initial = 1; 
%     eta = fsolve(@(eta)non_fun_eta(eta,xi,phi1_new,phi2_new,phi3_new,phi0),eta_initial,...
%                    opts);
